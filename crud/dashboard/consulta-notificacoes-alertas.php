...
// exclusiva aba que irá mostar os alertas de acordo com o padrão de notificações criadas em Coleções de Notificações de Tipo de Suporte x Usuário e Projetos que participa

if($usuario_sessao != null && $usuario_sessao != ''){

$sql = $conexao->query("SELECT s.*,sc.*, ts.nome_suporte, p.nome_ponto, e.nome_estacao 
                        FROM suporte s
                        ...
                        INNER JOIN notificacoes_usuario nf ON nf.id_obra = s.obra
                        WHERE s.status_suporte != '4' 

                        AND (sc.destinatario_direto = $usuario_sessao OR nf.id_usuario=$usuario_sessao) GROUP BY s.id_suporte");
