...
//Tubulação com Problemas 88
//Equipamento da Operação Danificado 89
//Instrumento de Medição Danificado 90

if($usuario_sessao != null && $usuario_sessao != ''){

    $tipo_suporte1 = 89; //Equipamento da Operação Danificado
    $tipo_suporte2 = 90; //Instrumento de Medição Danificado
    $tipo_suporte3 = 91; // Sistema Parado
    $tipo_suporte4 = 88; //Tubulação com Problemas


$sql = "SELECT s.*, ts.nome_suporte, p.nome_ponto, e.nome_estacao 
        FROM suporte s
        ...
        LEFT JOIN suporte_destinatarios sd ON sd.id_suporte_conversas = sc.id_conversa 
             AND sd.id_destinatario = :usuario_sessao
        WHERE s.status_suporte != '4' 
              AND s.tipo_suporte IN (:tipo_suporte1, :tipo_suporte2, :tipo_suporte3, :tipo_suporte4)
              AND (sc.destinatario_direto = :usuario_sessao OR sd.id_destinatario IS NOT NULL)";

$stmt = $conexao->prepare($sql);
$stmt->bindParam(':usuario_sessao', $usuario_sessao);
$stmt->bindParam(':tipo_suporte1', $tipo_suporte1);
$stmt->bindParam(':tipo_suporte2', $tipo_suporte2);
$stmt->bindParam(':tipo_suporte3', $tipo_suporte3);
$stmt->bindParam(':tipo_suporte4', $tipo_suporte4);
$stmt->execute();
...
<?php
 ...
 $conexao=null;
    ...
    <!--begin::Section-->
    <div class="d-flex align-items-center"><b>Nenhuma Notificação de Estrutural para:<br><br>
    - Tubulação com Problemas <br>
    - Equipamento da Operação Danificado <br>
    - Instrumento de Medição Danificado <br></b>
    </div></div>';
...
$conexao=null;
    ...
    echo '<script>window.location.replace("../../crud/login/logout.php");</script>';
}
