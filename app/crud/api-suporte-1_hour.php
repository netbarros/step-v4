...
function enviarSMS($numero_destino, $mensagem, $totalVoiceToken) {

    $mensagem_alerta_sem_tags = strip_tags($mensagem);
    ...
    if($response->getStatusCode() == 200) {
        ...
        echo 'Erro ao enviar SMS: '. $response->getContent();
    }
    
}

function enviarWhatsApp($numero_destino, $mensagem, $twilioAccountSid, $twilioAuthToken, $twilioFromNumber) {
    ...
    try {
        ...
        echo 'Erro ao enviar a mensagem do WhatsApp: ' . $e->getMessage();
    }
}
...
$periodo_verificacao = '-1 hour';//'-5 minutes';
        ...
        foreach ($agrupados as $idTipoSuporte => $obras) {
            foreach ($obras as $idObra => $linhas) {
                ...
                foreach ($usuarios_projeto as $usuario_projeto) {
                    ...
                    foreach ($linhas as $linha) {
                        ...
                        if($linha['id_rmm_suporte']!=0 && $linha['id_rmm_suporte']!=NULL){
                            ...
                            foreach ($indicadores as $indicador) {
                                $descricao .= $linha['motivo_suporte'].'<br>';
                            ...
                            $descricao = $linha['Nome_Tipo_Suporte'].' - '.$linha['motivo_suporte'];
                        }
                        ...
                        <p>Olá {$usuario_projeto['nome']},</p>
                        <p>Este é um alerta de notificação do Projeto: <strong>{$linha['Nome_Obra']}</strong>.</p>
                        <p>Uma nova relação de Alertas no sistema: <strong>{$linha['Nome_Tipo_Suporte']}</strong>.</p>
                        <p>Segue abaixo os dados das Notificicações:</p>"; 
                        
                        $url_curta = "| Acesse http://step.eco.br?t={$linha['Id_Tipo_Suporte']}&k={$linha['chave_unica']} |";
                        
                        $mensagem_curta = "Olá {$usuario_projeto['nome']},\n\nAlerta STEP, Projeto: {$linha['Nome_Obra']}.\n\n
                        {$linha['Nome_Tipo_Suporte']}.\n\r$url_curta";

                      // Aqui estou assumindo que você tem uma função chamada enviarNotificacoes.
                       //  enviarNotificacoes($usuario_projeto['id_usuario_projeto'], $assunto, $retorno_alerta);

           
                    
   
                        // Aqui vem o restante do seu código de formatação e envio de alertas:**
                    ...
                    <p>Você está recebendo este e-mail porque faz parte do Projeto:<strong> {$linha['Nome_Obra']} </strong> e possui Coleção Ativa de Notificação Personalizada para este projeto.</p>
                    <p>Para não receber mais este tipo de e-mail, acesse suas Coleções de Notificações no Sistema e Personalize por Projeto e {$linha['Nome_Tipo_Suporte']}.</p>
                </div>";

                    $retorno_alerta .= "<div style='font-family: Arial, sans-serif; padding: 20px; border-radius: 10px; background-color: #f2f2f2; color: #737373; line-height: 1.5; font-size: 11px;'>
                    <p>Este e-mail foi enviado automaticamente pelo Sistema de Gestão de Qualidade da EP Engenharia.</p>
                    <p>Para mais informações, entre em contato com a equipe de Qualidade da EP Engenharia - STEP.</p>";
                    ...
                    foreach ($notificacoes_usuario as $notificacao) {
                        ...
                        if ($notificacao['alerta_whats'] == 1) {
                            ...
                            enviarWhatsApp($telefone_usuario_alerta, $mensagem_curta, $twilioAccountSid, $twilioAuthToken, $twilioFromNumber);
                        }



                        if ($notificacao['alerta_email'] == 1) {
                            enviarEmail($usuario_projeto['email'], $assunto, $usuario_projeto['nome'], $mensagem_alerta, $retorno_alerta);
                        }
         ...
         $acao_log = "CRON Suporte Periodo 1 Hora";
