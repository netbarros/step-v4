    ...
    foreach ($resultado as $res) {
        ...
        if ($controle_concentracao == "1") { // controla a leitura mínima
            ...
            if ($leitura < $concen_min) {
                ...
                $conta_rmm = $sql_atualiza_status_rmm->rowCount();

                if($retorno_alerta!='' && $conta_rmm > 0){
                ...
                $teste=$conexao->query("DELETE FROM suporte WHERE id_rmm_suporte='$ID_RMM'");
...
INNER JOIN parametros_ponto pr ON pr.id_parametro = s.parametro WHERE r.status_leitura='1' AND s.status_suporte='1';");
