...
if(!empty($consulta) && password_verify($senha, $consulta->senha)){
  ...
  $_SESSION['id'] = $id_usuario;
    ...
    if(password_verify($senha, $consulta->senha)){
      ...
      setcookie('id_usuario_logado', $id_usuario, time() + (86400), "/"); 
...
//echo "O navegador usado é ".$navegador.", versão ".$versao;
                ...
                $retorno = array('codigo' => 1, 'id_usuario'=> $_SESSION['id'], 'nome'=> $nome, 'email' =>  $email, 'nivel' => $nivel, 'foto'=> $foto, 'status'=> $status, 'navegador'=>$navegador, 'versao'=>$versao);
            ...
            $retorno = array('codigo' => '0','mensagem'=>'Dados do Login inválidos! Verifique se realmente possui acesso ao sistema.');
             echo json_encode($retorno);
             exit();
