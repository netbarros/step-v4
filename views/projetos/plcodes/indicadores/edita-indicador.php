...
$_SESSION['modulo'] = 'Alterar Indicador';
...
if($id_indicador!=''){
	...
	$sql=$conexao->query("SELECT * FROM parametros_ponto pr WHERE pr.id_parametro = '$id_indicador'");
...
<form class="form" id="form_indicador" name="form_indicador">
	...
	<div class="card-body pt-9 pb-0">
		...
		<div class="mb-10 fv-row">
			...
			<div class="col-lg-12 form-group">
				...
				<input type="text" class="form-control " placeholder="Nome ou Identificação do Parâmetro" name="nome_parametro" maxlength="150" value="<?=$r['nome_parametro'];?>">
		...
		<div class="mb-10 fv-row">
			<div class="form-group">
				...
				<div class="col-lg-12 ">
					<select class="form-select mb-2 " data-control="select2" title="Selecione a Origem ..." data-dropdown-parent="#modal_altera_registro" name="origem_leitura_parametro" id="origem_leitura_parametro_add">
					<option></option>
					<option value="1" <?php if($r['origem_leitura_parametro']=='1') { echo "selected";} ;?>>
							Entrada
						</option>
						<option value="2" <?php if($r['origem_leitura_parametro']=='2') { echo "selected";} ;?>>
							Saída
						</option>
			...
			<div class="form-group" id="div_controla_concentracao_add">
				<label class="required form-label">
					Controle de Concentrações Permitidas:
				</label>
				<div class="col-lg-12 ">
					<select class="form-select mb-2" data-control="select2" title="Selecione o Controle de Concentração" data-dropdown-parent="#modal_altera_registro" id="controle_concentracao_add" name="controle_concentracao">
					<option></option>
						<option value="1" <?php if($r['controle_concentracao']=='1') { echo "selected";} ;?>>
							Concentração Mínima
						</option>
						<option value="2" <?php if($r['controle_concentracao']=='2') { echo "selected";} ;?>>
							Concentração Máxima
						</option>
						<option value="3" <?php if($r['controle_concentracao']=='3') { echo "selected";} ;?>>
			...
			<div id="div_controle_concentracao_1_add" class="d-flex flex-column d-none">
				...
				<div class="col-lg-6">
					<input type="text" class="form-control" id="concen_min_par" name="concen_min" placeholder="Mínima" value="<?=$r['concen_min'];?>">
				</div>
			...
			<div id="div_controle_concentracao_2_add" class="d-flex flex-column d-none">
				...
				<div class="col-6">
					<input type="text" class="form-control" id="concen_max_par" name="concen_max" placeholder="Máxima" value="<?=$r['concen_max'];?>">
				</div>
			...
			<div class="col-lg-4 form-group" id="div_unidade_medida_add">
				<label class="required form-label">Unidade de Medida</label>

				<div class="input-group">
					<select name="unidade_medida" id="unidade_medida_add" class="form-select mb-2" data-control="select2" data-size="4" data-dropdown-parent="#modal_altera_registro">
						...
						while ($laco = $stmt->fetch(PDO::FETCH_OBJ)) {
							...
							if (isset($r['unidade_medida']) == $laco->id_unidade_medida) :
								echo "<option value=" . $laco->id_unidade_medida . " selected>" . $laco->nome_unidade_medida . "</option>";
					...
					<div class="input-group-append">
						<button type="reset" class="btn btn-light-primary" id="exibe_unidade_medida_add">
							<i class="bi bi bi-bookmark-plus fs-1x"></i>
						</button>
			...
			<div class="col-lg-12 form-group d-none  alert alert-warning  align-items-center p-5" id="div_nova_unidade_medida_add">
				<label class="required form-label">Nova Unidade de Medida:</label>
				<div class="input-group">

					<input type="text" class="form-control" name="nova_unidade_medida" id="nova_unidade_medida_add" maxlength="10" placeholder="Nova Unidade">
					<div class="input-group-prepend">
						<button type="reset" class="btn btn-light-primary" id="inclui_unidade_medida_add">
							Incluir
						</button>

					</div>
					<span class="fs-7 text-muted">
						Nova Unidade de Medida <span id="form_help_nova_unidade_add"></span>
    ...
    <div id="div_controle_concentracao_2_add" class="d-flex flex-column">
      ...
      <div class="col-lg-8">
        <input type="text" class="form-control"  disabled="true" id="id_sensor_iot" name="id_sensor_iot" placeholder="id_sensor_iot" value="<?php echo $r['id_sensor_iot'] ?? ''?>">
      </div>
      <span class="fs-7 text-muted">
		...
		<div class="mb-10 fv-row">
				...
				<label class="form-check-label" for="">
						<input type="radio" value="3" class="form-check-input" name="status_parametro" <?php if($r['gera_grafico']=='3') { echo "checked";} ;?>>
						Inativo
						<span></span>
					</label>
					<label class="form-check-label" for="">
						...
						?>
						<input type="radio" value="1" class="form-check-input" name="status_parametro" <?php if($r['status_parametro']=='1') { echo "checked";} ;?>>
						Ativo
						<span></span>
					</label>
				</div>
			...
			<div class="form-group mb-10">
				...
				<div class="form-check form-check-custom form-check-solid">
				<label class="form-check-label" for="">
						<input type="radio" value="0" class="form-check-input" name="gera_grafico" <?php if($r['gera_grafico']=='0') { echo "checked";} ;?>>
						Não
						<span></span>
					</label>
					<label class="form-check-label" for="">
						<?php // Status 1 = Ativo || 0 =  Inativo 
						?>
						<input type="radio" value="1" class="form-check-input" name="gera_grafico" <?php if($r['gera_grafico']=='1') { echo "checked";} ;?>>
						Sim
						<span></span>
					</label>
				</div>
		...
		<input name="acao" type="hidden" value="altera_indicador">
	
		<input name="id"  type="hidden" value="<?=$r['id_parametro'];?>" id='id_indicador_atual'>
		<input name="id_plcode"  type="hidden" value="<?=$r['id_ponto'];?>" >
...
<script src="/js/indicadores/altera-indicador.js" type="text/javascript"></script>
<!--end::Form-->
