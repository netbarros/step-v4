...
ORDER BY s.data_open DESC";
    ...
    if ($stmt->execute()) {
        ...
        foreach ($agrupados as $idTipoSuporte => $obras) {
            foreach ($obras as $idObra => $linhas) {
                ...
                foreach ($usuarios_projeto as $usuario_projeto) {
                    ...
                    foreach ($linhas as $linha) {
                        ...
                        <p>Novos Tickets para: <strong>{$linha['Nome_Tipo_Suporte']}</strong>.</p>
                        <p>{$url_curta}</p></div>"; 
         ...
         echo "Houveram <b>".$contador."</b> Suportes, analisados e com Notificações enviadas de acordo com As Coleções criadas por usuário vs projeto.";
