...
function enviarSMS($telefone_usuario_alerta, $mensagem_curta, $totalVoiceToken) {
    ...
    if($response->getStatusCode() == 200) {
        error_log ('SMS enviado com sucesso '.$telefone_usuario_alerta.' - ' . $response->getContent());
    } else {

        echo 'Falha ao enviar SMS';
        error_log('Erro ao enviar SMS: '.$telefone_usuario_alerta.' - ' . $response->getContent());
    }
            ...
            foreach ($obras as $idObra => $linhas) {
                ...
                foreach ($usuarios_projeto as $usuario_projeto) {
                    ...
                    foreach ($linhas as $linha) {
                        ...
                        $tipo_suporte_mail = $linha['Id_Tipo_Suporte'];
                        $usuario_mail = $usuario_projeto['id_usuario_projeto'];

                        $tipo_suporte_mail = $tipo_suporte_mail;
                        ...
                        <p>Um novo ticket foi aberto no sistema: <strong>{$linha['Nome_Tipo_Suporte']}</strong>.</p>
                        <p>{$url_curta}</p></div>"; 
                    ...
                    $retorno_alerta .= "<div style='font-family: Arial, sans-serif; padding: 20px; padding-top: 20px;  border-radius: 10px; background-color: #f2f2f2; color: #737373; line-height: 1.5; font-size: 11px;'>
