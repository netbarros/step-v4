...
<?php
/* COM SMTP do step.eco.br configurado
// Path: cron\envia-email.php
// Configurações de timezone e locale
date_default_timezone_set('America/Sao_Paulo');
setlocale(LC_TIME, 'pt_BR', 'pt_BR.utf-8', 'portuguese');
...
try {
    ...
    // Configurações SMTP
    $mail->isSMTP(); // Use SMTP
    $mail->Host = 'webmail.step.eco.br'; // Set the SMTP server to send through
    $mail->SMTPAuth = true; // Enable SMTP authentication
    $mail->Username = 'webmaster@step.eco.br'; // SMTP username
    $mail->Password = 'grupoep123'; // SMTP password (change this)
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
